...
@frontend.route('/')
def home():
    time = Action.query.order_by(-Action.id).first_or_404().time.strftime('%H:%M, %b %d, %Y')
...
def meet_stats():
    ...
    time = time = Action.query.order_by(-Action.id).first_or_404().time.strftime('%H:%M, %b %d, %Y')
